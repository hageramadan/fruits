            ...
            {showVideo && embedVideoUrl ? (
              ...
              <>
                ...
                {discountPercentage > 0 && (
                  <span className="absolute top-2 right-2 bg-[#2ECC71] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full z-10">
              ...
              {allImages.map((image, index) => (
                <button
                  ...
                  className={`
                    ...
                    ${selectedImage === index ? "border-[#2ECC71]" : "border-transparent hover:border-gray-300"}
                  `}
                >
                  <Image
        ...
        <div className="space-y-2 lg:space-y-5 lg:col-span-3">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex flex-col items-end">
              <span className="text-lg lg:text-xl font-bold text-[#2ECC71] flex items-center gap-0.5">
          ...
          <div className="flex flex-col gap-1.5 lg:gap-4 mt-2 lg:mt-4">
            <button
              ...
              className={`flex-1 text-sm text-white px-4 py-2 rounded-[8px] font-bold flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300
                ${isAvailable ? 'bg-[#2ECC71] hover:bg-[#2ECC71]' : 'bg-gray-400 cursor-not-allowed'}
              `}
            >
              {isAddingToCart ? (
                <>
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={isMutating}
                className={`
                  flex-1 py-2 rounded-[8px] text-[#2ECC71] font-bold transition-all duration-300 flex items-center justify-center gap-2 text-xs
                  ${
                    isProductFavorite
                      ? "bg-red-50 text-red-600 border border-red-200 hover:bg-red-100"
                      : "border border-[#2ECC71] hover:bg-[#ff89e13f]"
                  }
                  disabled:opacity-50 disabled:cursor-not-allowed
                `}
              >
                <Heart
                  className={`h-3.5 w-3.5 ${isProductFavorite?'text-[#ef4444]':'text-[#2ECC71]'}`}
