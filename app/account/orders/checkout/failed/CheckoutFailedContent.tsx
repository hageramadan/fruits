      ...
      <div className="container-custom bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
        ...
        {orderNumber && (
          <p className="text-center text-gray-500 mb-2">
            {t('checkout.failed.orderNumber')}: <span className="font-bold text-[#2ECC71]">{orderNumber}</span>
          </p>
        )}
          ...
          <ul className="space-y-2">
            {suggestions.map((suggestion, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-gray-600">
                <span className="text-[#2ECC71] font-bold text-lg leading-none mt-0.5">•</span>
        ...
        <div className="flex flex-col gap-3">
          <button
            onClick={handleRetry}
            className="w-full bg-[#2ECC71] text-white py-3 rounded-xl font-medium hover:bg-[#a87aa8] transition flex items-center justify-center gap-2"
          >
